/*** Generated by Spin Version 6.2.7 -- 2 March 2014 ***/
/*** From source: 1 ***/
...
short src_ln0 [] = {
	  0,  31,  33,  34,  34,  11,  11,  11, 
	 12,  10,  14,   9,  15,  37,  37,  37, 
	 38,  38,  36,  40,  34,  40,  40,  40, 
	 41,  40,  42,  42,  42,  42,  43,  44, 
	 45,  45,  47,  19,  20,  18,  23,  47, 
	 47,  48,  48,  46,  50,  45,  50,  50, 
	 50,  51,  50,  41,  52,  52,  53,  32, 
	 55,  32,  55,  55,  57,  19,  20,  18, 
	 23,  57,  57,  58,  58,  56,  60,  55, 
	 60,  60,  60,  61,  60,  62,  62,  62, 
	 63,  63,  61,  65,  30,  66,  30,  66, 
	  0, };
S_F_MAP src_file0 [] = {
	{ "-", 0, 0 },
	{ "1", 1, 87 },
...
{	int j, h = now._nr_pr;
	...
	{	int l_in;
		...
		{
			((P0 *)pptr(h))->locked[l_in] = 0;
		}
	}
		((P0 *)pptr(h))->i = 0;
		((P0 *)pptr(h))->num = ((int)((P0 *)this)->_pid);
#ifdef VAR_RANGES
		logval("Worker:success", ((P0 *)pptr(h))->success);
		logval("Worker:i", ((P0 *)pptr(h))->i);
		logval("Worker:num", ((P0 *)pptr(h))->num);
	{	int l_in;
		for (l_in = 0; l_in < 5; l_in++)
		{
			logval("Worker:locked[l_in]", ((P0 *)pptr(h))->locked[l_in]);
		}
	}
#endif
#ifdef HAS_CODE
...
iniglobals(int calling_pid)
{
	...
	{	int l_in;
		...
		now.quorumsuccess = 0;
#ifdef VAR_RANGES
		logval("quorum", now.quorum);
	{	int l_in;
		for (l_in = 0; l_in < 5; l_in++)
		{
			logval("res[l_in]", now.res[l_in]);
		}
	}
...
{	/* int i; */
	...
	printf("	int    anysuccess:	%d\n", now.anysuccess);
	printf("	int    quorumsuccess:	%d\n", now.quorumsuccess);
}
void
...
{	/* int i; */
	...
	printf("	int    success:	%d\n", ((P0 *)pptr(pid))->success);
	{	int l_in;
		for (l_in = 0; l_in < 5; l_in++)
		{
			printf("	int    locked[%d]:	%d\n", l_in, ((P0 *)pptr(pid))->locked[l_in]);
		}
	}
	printf("	int    i:	%d\n", ((P0 *)pptr(pid))->i);
	printf("	int    num:	%d\n", ((P0 *)pptr(pid))->num);
